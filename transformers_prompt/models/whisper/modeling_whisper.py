...
class WhisperEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # hidden_states = hidden_states + self.adapter_en_1(hidden_states)
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        # hidden_states = hidden_states + self.adapter_en_2(hidden_states)
        hidden_states = residual + hidden_states
...
class WhisperDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        # hidden_states = hidden_states + self.adapter_de_1(hidden_states)
        hidden_states = residual + hidden_states
        ...
        hidden_states = self.fc2(hidden_states)
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        # hidden_states = hidden_states + self.adapter_de_2(hidden_states)
        hidden_states = residual + hidden_states
...
class WhisperPromptEncoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig):
        super().__init__()
        self.embed_dim = config.d_model
...
# Copied from transformers.models.mbart.modeling_mbart.MBartDecoderLayer with MBart->Whisper, MBART->WHISPER
class WhisperPromptDecoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig):
        super().__init__()
        self.embed_dim = config.d_model
...
class WhisperPromptEncoder(WhisperPreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
    [`WhisperEncoderLayer`].
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.layers = nn.ModuleList([WhisperPromptEncoderLayer(config) for _ in range(config.encoder_layers)])
        self.layer_norm = nn.LayerNorm(config.d_model)
...
class WhisperPromptDecoder(WhisperPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`WhisperDecoderLayer`]
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.layers = nn.ModuleList([WhisperPromptDecoderLayer(config) for _ in range(config.decoder_layers)])
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
        self._use_sdpa = config._attn_implementation == "sdpa"
    ...
    def forward(
        ...
        # if prompt_ids is not None and prompts_embeds is not None:
        #     raise ValueError("You cannot specify both decoder_input_ids and decoder_inputs_embeds at the same time")
        # elif prompt_ids is not None:
        #     prompt_shape = prompt_ids.size()
        #     prompt_ids = prompt_ids.view(-1, prompt_shape[-1])
        # elif prompts_embeds is not None:
        #     prompt_shape = prompts_embeds.size()[:-1]
        # else:
        #     raise ValueError("You have to specify either decoder_input_ids or decoder_inputs_embeds")
        ...
        #     prompts_embeds = self.embed_tokens(prompt_ids)

        # prompts_inputs_embeds = None
...
class WhisperPromptModel(WhisperPreTrainedModel):
    def __init__(self, config: WhisperConfig):
        super().__init__(config)

        self.encoder = WhisperPromptEncoder(config)
        self.decoder = WhisperPromptDecoder(config)
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids,
            # prompt_ids=decoder_prompts_ids,
            attention_mask=decoder_attention_mask,
            encoder_hidden_states=encoder_outputs[0],
            head_mask=decoder_head_mask,
            cross_attn_head_mask=cross_attn_head_mask,
