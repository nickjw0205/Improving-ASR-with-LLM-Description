...
class GenerationMixin:
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        if model_kwargs['prompts'] is not None:
            if len(model_kwargs['prompts']) != 1:
                raise ValueError("Only one prompt is supported for encoder-decoder prompt models. change batch to 1")
            # <|startofprev|> prompt <|startoftranscript|>
            decoder_start_token_id = [self.config.pos_token_id] + model_kwargs['prompts'][0].tolist() + [self._get_decoder_start_token_id(decoder_start_token_id, bos_token_id)]
            if device is None:
                device = self.device
            decoder_input_ids_start = torch.tensor([decoder_start_token_id], device=device)
        else:
            decoder_start_token_id = self._get_decoder_start_token_id(decoder_start_token_id, bos_token_id) # 50257
            if device is None:
                device = self.device
        ...
        **kwargs,
        ...
        if model_kwargs.get("attention_mask", None) is None and requires_attention_mask and accepts_attention_mask:
            model_kwargs["attention_mask"] = self._prepare_attention_mask_for_generation(
                inputs_tensor, generation_config.pad_token_id, generation_config.eos_token_id # remove attention for pad
            )
